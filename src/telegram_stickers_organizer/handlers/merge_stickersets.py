...
async def merge_stickersets(callback_query: CallbackQuery, state: FSMContext) -> None:
    ...
    await bot.send_message(
        user_id,
        "Choose a stickerset to merge into:",
        reply_markup=keyboard,
    )
    await state.set_state(MergeStates.waiting_for_first_sticker)


@router.callback_query(StateFilter(MergeStates.waiting_for_first_sticker))
async def process_first_sticker(
    callback_query: CallbackQuery, state: FSMContext
) -> None:
    sticker_set_name = callback_query.data.split(":")[1]
